...
class Mods_List_Table extends WP_List_Table
{
	...
	function __construct( $child_slug, $parent_slug )
	{
		...
		parent::__construct( array(
			'singular' => __( 'key', ITM_TEXT_DOMAIN, 'inherit-theme-mods' ),
			'plural'   => __( 'keys', ITM_TEXT_DOMAIN, 'inherit-theme-mods' ),
			'ajax'     => true,
		) );
	}
	...
	public function prepare_items()
	{
		...
		$per_page = ITM_ADMIN_LIST_PER_PAGE;
	...
	function generate_data_array()
	{
		...
		function transform_mod( $key, $mods, $col )
		{
			...
			if( $match_color === 1 ) {
				...
				$value = esc_url( $value );
				$value = "<div class=\"ITM-color-indication\" $styleAttr></div><span class=\"ITM-list-data\" $data_key $data_col>$value</span>";
